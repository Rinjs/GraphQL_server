...
const resolvers = {
  Query: {
    user: (parent, {
      id
    }, context, info) => _db__WEBPACK_IMPORTED_MODULE_0__["users"].find(user => user.id == id),
    users: (parent, args, context, info) => _db__WEBPACK_IMPORTED_MODULE_0__["users"]
  },
  Mutation: {
    createUser: (parent, {
      id,
    ...
    updateUser: (parent, newDate, context, info) => {
      let newUser = _db__WEBPACK_IMPORTED_MODULE_0__["users"].find(user => user.id == newDate.id);
